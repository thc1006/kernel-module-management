# Helpers for a test that holds an object with a finalizer of its own. metadata.finalizers is a
# list and a merge patch sends it whole, so these rewrite it under the resourceVersion they read.
