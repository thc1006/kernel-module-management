...
var _ = Describe("PluginReconcilerNodePredicate", func() {
	It("should return true for creations", func() {
		ev := event.CreateEvent{
			Object: &v1.Node{},
		}
		...
		Expect(
			PluginReconcilerNodePredicate().Create(ev),
		).To(
			BeTrue(),
		)
	})
	...
	It("should return true when taints change", func() {
		...
		Expect(
			PluginReconcilerNodePredicate().Update(ev),
		).To(
			BeTrue(),
		)
	})
	...
	It("should return false for deletions", func() {
		...
		Expect(
			PluginReconcilerNodePredicate().Delete(ev),
		).To(
			BeFalse(),
		)
	})
...
var _ = Describe("FindDevicePluginModulesForNode", func() {
	BeforeEach(func() {
		mockCtrl = gomock.NewController(GinkgoT())
		clnt = mockClient.NewMockClient(mockCtrl)
		f = New(clnt, nil)
	...
	It("should return the module owning a device-plugin-target label and ignore DRA-only modules", func() {
		clnt.EXPECT().List(ctx, gomock.Any(), gomock.Any()).DoAndReturn(
			func(_ interface{}, list *kmmv1beta1.ModuleList, _ ...interface{}) error {
				list.Items = []kmmv1beta1.Module{
					{
						ObjectMeta: metav1.ObjectMeta{Name: "dra-only", Namespace: "some-namespace"},
						Spec: kmmv1beta1.ModuleSpec{
							Selector: map[string]string{"worker": "true"},
							DRA:      &kmmv1beta1.DRASpec{},
						},
					},
				}
		...
		node := v1.Node{
			ObjectMeta: metav1.ObjectMeta{
				Labels: map[string]string{
					"worker": "true",
					"kmm.node.kubernetes.io/some-namespace.dp-module.device-plugin-target": "",
				},
			},
		}

		Expect(f.FindDevicePluginModulesForNode(ctx, &node)).To(ConsistOf(
			reconcile.Request{NamespacedName: types.NamespacedName{Namespace: "some-namespace", Name: "dp-module"}},
		))
	})
})
