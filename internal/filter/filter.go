...
var modulePodSuccess predicate.Predicate = predicate.Funcs{
	UpdateFunc: func(e event.UpdateEvent) bool {
...
// PluginReconcilerNodePredicate matches the node events the device plugin and the DRA reconcilers
// act on: creations, taint changes, and label changes. Label changes matter because a node that
// stops matching a Module's selector must have its target label removed, and because the target
// label itself is what both reconcilers converge on.
func PluginReconcilerNodePredicate() predicate.Predicate {
	return predicate.And(
		skipDeletions,
		predicate.Or(nodeTaintsChanged, predicate.LabelChangedPredicate{}),
	)
...
// needs its DRA driver, keeping the ones where the allocation appears or names different devices,
// or where the set of consumers holding the claim grows or shrinks. Creations and deletions are
// always passed on, since both change which claims exist.
...
func (f *Filter) FindDRAModulesForNode(ctx context.Context, node client.Object) []reconcile.Request {
	return f.findPluginModulesForNode(
		ctx,
		node,
		utils.IsDRATargetNodeLabel,
		func(mod *kmmv1beta1.Module) bool { return mod.Spec.DRA != nil },
	)
}

// FindDevicePluginModulesForNode is the device plugin counterpart of FindDRAModulesForNode.
func (f *Filter) FindDevicePluginModulesForNode(ctx context.Context, node client.Object) []reconcile.Request {
	return f.findPluginModulesForNode(
		ctx,
		node,
		utils.IsDevicePluginTargetNodeLabel,
		func(mod *kmmv1beta1.Module) bool { return mod.Spec.DevicePlugin != nil },
	)
}
...
func (f *Filter) findPluginModulesForNode(
	ctx context.Context,
	node client.Object,
	isTargetLabel func(string) (bool, string, string),
	hasPluginSpec func(*kmmv1beta1.Module) bool,
) []reconcile.Request {
	logger := ctrl.LoggerFrom(ctx).WithValues("node", node.GetName())

	reqSet := sets.New[reconcile.Request]()

	// A target label names the Module that owns it, so these do not need a Module list and are
	// found even when the Module no longer selects this node.
	for label := range node.GetLabels() {
		if ok, ns, name := isTargetLabel(label); ok {
