...
var _ = Describe("IsDevicePluginTargetNodeLabel", func() {
	DescribeTable("should work as expected",
		func(label string, expectedOK bool, expectedNS, expectedName string) {
			ok, ns, name := IsDevicePluginTargetNodeLabel(label)
			Expect(ok).To(Equal(expectedOK))
			Expect(ns).To(Equal(expectedNS))
			Expect(name).To(Equal(expectedName))
		},
		Entry("device-plugin-target label", "kmm.node.kubernetes.io/some-namespace.some-name.device-plugin-target", true, "some-namespace", "some-name"),
		Entry("device-plugin-ready label", "kmm.node.kubernetes.io/some-namespace.some-name.device-plugin-ready", false, "", ""),
		Entry("dra-target label", "kmm.node.kubernetes.io/some-namespace.some-name.dra-target", false, "", ""),
		Entry("unrelated label", "some-label", false, "", ""),
	)
})
